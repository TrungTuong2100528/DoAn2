...
<?php
  ...
  $sql ="SELECT sp.*, km.GiaKhuyenMai
        FROM SanPham sp 
        LEFT JOIN KhuyenMai km ON sp.IDSanPham = km.IDSanPham";
        $query = mysqli_query($con, $sql);
        $giam = mysqli_fetch_array($query);
?>
<?php
      session_start();
...
.dropdown .dropdown:hover .dropdown a {background-color: #f1f1f1;}
  ...
  <style>
    /* CSS cho sản phẩm không giảm giá */
    .item {
    ...
    /* CSS cho sản phẩm được giảm giá */
    .item.discounted {
        border: 2px solid #ff0000; /* Viền đậm màu đỏ */
        background-color: #ffe6e6; /* Nền màu hồng nhạt */
        box-shadow: 0 0 10px rgba(0, 0, 0, 0.2); /* Đổ bóng */
    }

    /* Kí hiệu cho sản phẩm được giảm giá */
    .item.discounted:before {
        content: "ĐÃ GIẢM GIÁ"; /* Hiển thị chữ "GIẢM GIÁ" trước sản phẩm */
        display: block;
        ...
        border-radius: 5px;
        position: absolute;
        top: -20px; /* Điều chỉnh vị trí top để căn đúng */
        left: 50%; /* Điều chỉnh vị trí left để căn giữa */
        transform: translateX(-50%) rotate(-45deg); /* Xoay chữ và căn giữa */
        z-index: 1;
    }
</style>
</style>
...
<body>
  ...
  <div class="w3-padding w3-xlarge w3-text-orange">
   ...
   ?>
    ...
    <!-- <div  class="dropdown">
      <a href="/WEB_TBYT/giaodienkhachhang/">Các sản phẩm khuyến mãi</a>
    </div> -->
  ...
  <!-- <div class="GioHang">
    <span class="badge bg-danger" id="qty">0</span>
  ...
  <div class="row">
    ...
    <?php
    $sql_pro = "SELECT sp.*, km.GiaKhuyenMai, km.NgayBatDau, km.NgayKetThuc
                FROM SanPham sp
                LEFT JOIN KhuyenMai km ON sp.IDSanPham = km.IDSanPham
                WHERE 1";
    ...
    if (isset($_GET['timkiem'])) {
        $tukhoa = $_GET['tukhoa'];
        $sql_pro .= " AND sp.TenSanPham LIKE '%" . $tukhoa . "%'";
    }

    if (isset($_GET['danhmuc'])) {
        $danhmuc = $_GET['danhmuc'];
        $sql_pro .= " AND sp.DanhMuc = '" . $danhmuc . "'";
    }
    ...
    while ($row = mysqli_fetch_array($query_pro)) {
        $gia = $row["GiaSanPham"];
        if (!empty($row["GiaKhuyenMai"])) {
            $today = date("Y-m-d");
            if ($row["NgayBatDau"] <= $today && $today <= $row["NgayKetThuc"]) {
                $gia_khuyenmai = $gia - $row["GiaKhuyenMai"];
                $discounted_class = 'discounted';
            } else {
                $gia_khuyenmai = $gia;
                $discounted_class = '';
            }
...
?>
    ...
    <?php
        ...
        $listc_km = "SELECT sp.*, km.GiaKhuyenMai AS GiaKM
                    FROM SanPham sp
                    JOIN KhuyenMai km ON sp.IDSanPham = km.IDSanPham
                    WHERE km.NgayBatDau <= '$current_date' AND km.NgayKetThuc >= '$current_date'";
        $result_km = mysqli_query($con, $listc_km);

        while ($row = mysqli_fetch_array($result_km)) {
        ?>
            <div class="item" style="
                ...
                flex-direction: column;
                justify-content: space-between;">
                <img style="max-width: 100%; max-height: 200px; margin-bottom: 20px;" src="/WEB_TBYT/giaodienkhachhang/image_giaodien/<?php echo $row['HinhAnh']; ?>" alt="">
                <a class="name" style="margin-bottom: -10px;"><?php echo $row["TenSanPham"] ?></a>
               
                <a style="margin-top: 10px;">Giá gốc: <?php echo number_format($row["GiaSanPham"], 0, ',', '.');?> VNĐ <br> Giảm : <?php echo number_format($row["GiaKM"], 0, ',', '.');?> VNĐ</a>
            </div>
        <?php
        }
        ?>
  ...
  <script>
      ...
      //     $.post("shoppingcart.php",{'IDSanPham':IDSanPham}, function(data, status){
      //       item = data.split("-");
      //       $("#qty").text(item[0]);
      //       $("#total").text(item[1]);
      //     });
