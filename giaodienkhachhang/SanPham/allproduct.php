<?php
    ...
    if ($query) {
      $dssp = mysqli_fetch_all($query, MYSQLI_ASSOC);
  } else {
      $dssp = [];
  }
?>  
...
foreach ($dssp as $sanpham) {
    ...
    if (file_exists($_SERVER['DOCUMENT_ROOT'] . $imgPath)) {
        // Hiển thị ảnh
        echo '<div class="item">';
        echo '<img src="' . $imgPath . '" alt="' . $tenSanPham . '">';
        echo '<a class="name">' . $tenSanPham . '</a>';
        echo '<a href="../ThongTinSanPham/ThongTinSanPham.php"> <button>Xem</button></a>';
        echo '</div>';
    } else {
        // Hiển thị thông báo nếu ảnh không tồn tại
        echo '<div class="item">';
