<?php
    ...
    if(isset($_GET['id'])){
        ...
        if(isset($_POST['sbm'])){
            ...
            // Check if username or email already exists in other accounts
            $sql_check = "SELECT * FROM KhachHang WHERE (UserName='$UserName' OR Email='$Email') AND IDKhachHang != $id";
            $query_check = mysqli_query($con, $sql_check);
            if(mysqli_num_rows($query_check) > 0){
                echo "<script>alert('Tên đăng nhập hoặc Email đã tồn tại ở các tài khoản khác.');</script>";
            } else {
                $today = date("Y-m-d");
                if ($NgaySinh >= $today) {
...
<body>
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <input type="hidden" name="IDKhachHang" value="<?php echo $row_select['IDKhachHang']; ?>">
