...
if (isset($_POST['sbm'])) {
    ...
    // Kiểm tra tên đăng nhập và email đã tồn tại trong cơ sở dữ liệu
    $check_sql = "SELECT * FROM `Admin` WHERE UserName='$UserName' OR Email='$Email'";
    $check_query = mysqli_query($con, $check_sql);
    if (mysqli_num_rows($check_query) > 0) {
        // Hiển thị thông báo nếu tên đăng nhập hoặc email đã tồn tại
        echo "<script>alert('Tên đăng nhập hoặc Email đã tồn tại.');</script>";
    } else {
        // Thêm admin vào cơ sở dữ liệu nếu chưa tồn tại
        $sql = "INSERT INTO `Admin`( UserName, Pass, HoTen, Email, SDT, NgaySinh, DiaChi) 
