...
<?php
        ...
        // Truy vấn để lấy thông tin từ bảng TrangThaiDonHang
        $sql = "SELECT * FROM `TrangThaiDonHang`";
        $query = mysqli_query($con, $sql);
    
        // Truy vấn để cập nhật trạng thái đơn hàng từ bảng DonHang
        $update_sql = "UPDATE TrangThaiDonHang t
                       JOIN DonHang d ON t.IDDonHang = d.IDDonHang
                       SET t.TenTrangThai = d.TinhTrang";
        ...
        if (mysqli_query($con, $update_sql)) {
            ...
            echo "<script>alert('Lỗi khi cập nhật trạng thái đơn hàng: " . mysqli_error($con) . "');</script>";
        }
?>
<head>
        <meta charset="UTF-8">
...
<div class="contrainer-fluid">
    <div class="card">
        ...
        <li class="dropdown">
                ...
                <li><a  style="color: #f00;" href="../trangthaidonhang/trangthaidonhang.php">Quản lý trạng thái đơn hàng</a></li>
            ...
            <table class="table">
                ...
                <?php
                    ...
                    while($row = mysqli_fetch_assoc($query)){?>
                        <tr>
                            <td><?php echo $i++; ?></td>
                            <td><?php echo isset($row['IDTrangThaiDonHang']) ? $row['IDTrangThaiDonHang'] : ''; ?></td>
                            <td><?php echo isset($row['IDDonHang']) ? $row['IDDonHang'] : ''; ?></td>
                            <td><?php echo isset($row['TenTrangThai']) ? $row['TenTrangThai'] : ''; ?></td>
                            <?php if(isset($_SESSION['dangnhap_nv'])) { ?>
                                <!-- <td><a href="sua_trangthaidonhang.php?id=<?php echo $row['IDTrangThaiDonHang']; ?>">Sửa</a></td> -->
                                <td>Không có quyền</td>
                            <?php } else { ?>
                                <!-- <td><a href="sua_trangthaidonhang.php?id=<?php echo $row['IDTrangThaiDonHang']; ?>">Sửa</a></td> -->
                                <td><a href="xoa_trangthaidonhang.php?id=<?php echo $row['IDTrangThaiDonHang']; ?>" onclick="return confirm('Bạn có chắc chắn muốn xóa trạng thái đơn hàng này?')">Xóa</a></td>
                            <?php } ?>
